...
var FAPI = {
  ...
  swfCallback : function (e)  {
    if (e.success){
      FAPI.flash = document.getElementById("FAPI_Flash");
  ...
  embedFlash : function(success, failure){
    ...
    window.fapi_success = this.isFunc(success) ? success : function(){};
    window.fapi_failure = this.isFunc(failure) ? failure : function(){};

    swfobject.embedSWF(this.serverUrl+"flash/ForticomAPIJS.swf?v=8", "FAPI_Flash_wrap", "1", "1", "9.0.0", this.serverUrl+"flash/expressInstall.swf", flashVars, flashParams, flashAttributes, this.swfCallback);
  ...
  invokeUIMethod: function (methodName)
  {
    /*
    ...
    var r = [];
        for (var i in args) {
        var arg;
        //only string arguments supported for now
    ...
    var expr = "this.getFlash().FAPI_send("+r.join(',')+");";
    eval(expr);
    */
    ...
    this.getFlash().FAPI_send.apply(this.getFlash(), arguments);
...
FAPI.UI = {
    ...
    showInvite : function (text, params)
    {
      FAPI.invokeUIMethod("showInvite", text, params);
    },
    showNotification : function (text, params)
    {
      FAPI.invokeUIMethod("showNotification", text, params);
    },
